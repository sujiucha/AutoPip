...
**智能扫描与守护**
- [x] 自动扫描 `plugins` 目录下所有的 `requirements.txt` 依赖清单，支持自定义最大扫描深度。
- [x] 开机静默守护：服务器启动时自动在后台核对依赖，若发现缺失，将延迟在控制台底部高亮报警，防患于未然。
...
**交互体验优化**
- [x] 智能分流输出：游戏内查询列表时仅展示核心结果防止刷屏，并提供 RText 动作回调点击；后台控制台查询时则输出完整清单。
...
插件首次加载后，会自动在 `config/autopip` 生成配置文件。修改后输入 `!!pip reload` 即可热重载配置。
